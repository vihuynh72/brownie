...
describe('describePayload', () => {
  ...
  it('names files copied from Google Drive as a place a value may come from', () => {
    const text = describePayload({ kind: 'ALLOWED_SOURCE_KINDS', fieldId: 'summary', allowedSourceKinds: ['GOOGLE_DRIVE'] })

    expect(text).toBe('summary: only allow values from files copied from Google Drive')
  })
