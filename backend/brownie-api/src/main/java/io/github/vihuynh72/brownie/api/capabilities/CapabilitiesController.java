 ...
 * finalize, plus which of those Assist can read as a source today. The
 * values come from the same configuration the upload route enforces, so
 * the number a person sees is the number that will be applied.
 */
@RestController
...
class CapabilitiesController {
    ...
    CapabilitiesController(@Value("${brownie.artifacts.max-upload-bytes:10485760}") long maxUploadBytes) {
