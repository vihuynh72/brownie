...
/** Proves the plain-column round trip, nullable PDF pair, and row-level security of {@code export_receipt}, mirroring {@code JdbcExportApprovalRepositoryTest}'s own shape. */
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.NONE)
@ActiveProfiles("test")
@Testcontainers
class JdbcExportReceiptRepositoryTest {
    ...
    @Test
    void savedReceiptRoundTripsEveryColumnIncludingTheNullablePdfPair() {
        long userId = newUser("subject-export-receipt-round-trip").id();
        ...
        long approvalId = insertExportApproval(workspaceId, userId, documentId, revisionId, activated, manifestId);
        long docxArtifactId = insertArtifact(workspaceId, userId);

        assertThat(exportRepository.findLatest(workspaceId, userId, documentId)).isEmpty();

        ExportReceipt receipt = exportRepository.save(
                workspaceId, userId, documentId, revisionId, activated.id(), approvalId, manifestId,
                docxArtifactId, "d".repeat(64), null, null, ExportFormat.DOCX);

        assertThat(receipt.isCompletePair()).isFalse();
        ExportReceipt latest = exportRepository.findLatest(workspaceId, userId, documentId).orElseThrow();
        assertThat(latest.id()).isEqualTo(receipt.id());
        assertThat(latest.docxArtifactId()).isEqualTo(docxArtifactId);
        assertThat(latest.pdfArtifactId()).isNull();
        assertThat(latest.exportApprovalId()).isEqualTo(approvalId);
        assertThat(latest.validationManifestId()).isEqualTo(manifestId);
        assertThat(latest.format()).isEqualTo(ExportFormat.DOCX);
    }

    @Test
    void oneUsersContextCannotReadAnotherWorkspacesExportReceipt() {
        UserIdentity userA = newUser("subject-export-receipt-rls-a");
        UserIdentity userB = newUser("subject-export-receipt-rls-b");
        ...
        long approvalId = insertExportApproval(workspaceBId, userB.id(), documentId, revisionId, activated, manifestId);
        long docxArtifactId = insertArtifact(workspaceBId, userB.id());

        exportRepository.save(
                workspaceBId, userB.id(), documentId, revisionId, activated.id(), approvalId, manifestId,
                docxArtifactId, "e".repeat(64), null, null, ExportFormat.DOCX);

        assertThat(exportRepository.findLatest(workspaceBId, userA.id(), documentId)).isEmpty();
    }
    ...
    private UserIdentity newUser(String subject) {
        return userIdentityRepository.recordLogin("https://issuer-export-receipt-tests", subject, null, null);
    }
    ...
    private long insertExportApproval(
            long workspaceId, long userId, long documentId, long revisionId, TemplateVersion templateVersion, long manifestId) {
        try (Connection connection = dataSource.getConnection()) {
            connection.setAutoCommit(false);
            setLocalContext(connection, userId);
            try (PreparedStatement statement = connection.prepareStatement(
                ...
                statement.setLong(2, documentId);
                statement.setLong(3, revisionId);
                statement.setLong(4, templateVersion.id());
                statement.setLong(5, manifestId);
